# Code Modified BY Weiqi Yan 2024
...
class CfgNode(dict):
    ...
    @classmethod
    def load_with_base(
        cls, filename: str, new_allowed: bool = True
    ) -> Dict[str, Any]:
        """Load a cfg from file with non structure, and inherit attributes \
            from its `_BASE_`.

        Args:
            filename (str or file-like object): the file name or file of the current config.
                Will be used to find the base config file.
            new_allowed (bool): allow to create new key

        Returns:
            (dict): the loaded cfg
        """
        cfg = cls(new_allowed=new_allowed)
        cfg.defrost()
        ...
        def _load_with_base(base_cfg_file: str) -> Dict[str, Any]:
            ...
            return cls.load_with_base(base_cfg_file, new_allowed=new_allowed)
