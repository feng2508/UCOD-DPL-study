...
while getopts "c:p:g:m:r:" opt; do
  case "$opt" in
    ...
    ?) echo "Usage: $0 [-c config_file] [-p master_port] [-g gpus] [-m model_path] [-r refiner_path]"; exit 1 ;;
  esac
done
...
if [ -z "$REFINER_PATH" ]; then
  echo "Error: Refiner path (-rm) is required."
  exit 1
fi
...
OPTS+=" --refiner_path $REFINER_PATH"
...
CMD="accelerate launch $DISTRIBUTED_ARGS scripts/LTeval.py $OPTS"
